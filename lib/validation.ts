...
export const settingsSchema = z.object({
  ...
  serviceChargeType: z.enum(['PERCENT', 'AMOUNT']).optional(),
  serviceChargeValue: z.number().min(0).max(1000000).optional(),
  serviceChargeVatRate: z.number().min(0).max(100).optional(),
  waiterDiscountLimitPct: z.number().min(0).max(100).optional(),
  twoFactorRequiredRoles: z.array(z.enum(['OWNER', 'EMPLOYEE'])).max(2).optional(),
